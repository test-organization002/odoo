...
class SiparisDurum(models.Model):
    _name = 'siparis.durum'
    _description = 'Sipariş Durum Yönetimi'
    ...
    # İlişkiler
    terminal_ids = fields.One2many('terminal.management', 'siparis_durum', string='Terminaller')
    terminal_count = fields.Integer(string='Terminal Sayısı', compute='_compute_terminal_count')
    
    _sql_constraints = [
        ('code_uniq', 'unique(code)', 'Sipariş durum kodu benzersiz olmalıdır!'),
        ('name_uniq', 'unique(name)', 'Sipariş durum adı benzersiz olmalıdır!')
    ]
    ...
    def action_view_terminals(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'domain': [('siparis_durum', '=', self.id)],
            'context': {'default_siparis_durum': self.id},
        }
