...
class PosDurum(models.Model):
    _name = 'pos.durum'
    _description = 'POS Durum Yönetimi'
    ...
    # İlişkiler
    terminal_ids = fields.One2many('terminal.management', 'pos_durum', string='Terminaller')
    terminal_count = fields.Integer(string='Terminal Sayısı', compute='_compute_terminal_count')
    
    _sql_constraints = [
        ('code_uniq', 'unique(code)', 'POS durum kodu benzersiz olmalıdır!'),
        ('name_uniq', 'unique(name)', 'POS durum adı benzersiz olmalıdır!')
    ]
    ...
    def action_view_terminals(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'domain': [('pos_durum', '=', self.id)],
            'context': {'default_pos_durum': self.id},
        }
